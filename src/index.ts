...
export function createAsyncClient(options: ClientOptions): SleekAsyncClient | any {
  const { siteToken, env = 'latest', lang, cacheMinutes } = options;
  ...
  async function getContent(search?: string): Promise<SleekSiteContent | null> {
    if (!search && !syncClient) {
      syncClient = await createSyncClient({ siteToken, env, flush, lang, cache });
    }
    if (syncClient) return syncClient.getContent(search);
    if (!search) return null; // unlikely
    
    return await fetchSiteContent({ siteToken, env, search, lang, cache, cacheMinutes, flush }) as SleekSiteContent;
  }
  ...
  async function getPages(path: string, options?: GetPagesOptions): Promise<SleekSiteContent["pages"]> {
    ...
    const pages = await fetchSiteContent({ siteToken, env, search: 'pages', lang, cache, cacheMinutes, flush }) as SleekSiteContent["pages"];
  ...
  async function getEntry(handle: string): Promise<Entry | Entry[] | null> {
    ...
    return await fetchSiteContent({ siteToken, env, search, lang, cache, cacheMinutes, flush }) as Entry | Entry[] | null;
  }
  ...
  async function getImage(name: string): Promise<Image | null> {
    ...
    const images = await fetchSiteContent({ siteToken, env, search: 'images', lang, cache, cacheMinutes, flush }) as Record<string, Image>;
  ...
  async function getOptions(name: string): Promise<Options | null> {
    ...
    const options = await fetchSiteContent({ siteToken, env, search: 'options', lang, cache, cacheMinutes, flush }) as Record<string, Options>;
    const optionSet = options[name];
    return Array.isArray(optionSet) ? optionSet : null;
  }
